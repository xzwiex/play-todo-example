...
class ControllersSpec  extends Specification {
  ...
  "Controllers" should {
    ...
    "todo requests without auth & body should return 403/415"  in new WithApplication(FakeAppGenerator.application) {


      route(app, FakeRequest(GET, "/todo/list")) must beSome.which (status(_) == FORBIDDEN)

      route(app, FakeRequest(PUT, "/todo/add")) must beSome.which (status(_) == UNSUPPORTED_MEDIA_TYPE)

      route(app, FakeRequest(POST, "/todo/update")) must beSome.which (status(_) == UNSUPPORTED_MEDIA_TYPE)
